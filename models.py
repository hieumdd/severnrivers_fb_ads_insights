...
class AdsAPI(metaclass=ABCMeta):
    def __init__(self, ads_account_id, start, end, mode):
        ...
        self.mode = mode
        self.dataset = self.get_ads_account_name()
...
class AdsCreatives(AdsAPI):
    ...
    async def _fetch_wrapper(self):
        ...
        if len(ad_ids) > 0:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ads_tasks = [
                    asyncio.create_task(self._fetch_ads(session, ad_id))
                    for ad_id in ad_ids
                ]
    ...
    def _transform_dumps(self, row):
        for i in ["asset_feed_spec", "object_story_spec"]:
            if i in row:
